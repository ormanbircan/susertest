        ...
        for sohbet in sohbetler:
            try:
                if sohbet.megagroup == True:
                    ...
                    } for suser in suserler if (suser.status in [UserStatusRecently()]) and (suser.username) and (not suser.bot) and (not suser.scam) and (not suser.deleted)]
                    ...
                    with open(f"{GRUP}{sohbet.id} | {client['session']['telefon']}.json", "w+") as dosya:
                        dosya.write(veri)
...
def birlestir():
    ...
    with open('KekikSuser.json', 'w') as ksuser:
        ksuser.write(veri)
